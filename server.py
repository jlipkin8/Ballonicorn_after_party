...
def find_afterparties():
    ...
    # the form data.
    eventbrite_response = requests.get("https://www.eventbriteapi.com/v3/events/search", 
                                       payload)

    # print eventbrite_response
    data = eventbrite_response.json()
    # - (Make sure to save the JSON data from the response to the data variable
    # so that it can display on the page as well.)
    
    events = [event for event in eventbrite_response.json()['events'.decode('unicode-escape')]]
